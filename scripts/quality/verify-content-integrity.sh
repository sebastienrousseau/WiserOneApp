...
score_hits="$(rg -n '\b[0-9]+/[0-9]+\b' $targets 2>/dev/null || true)"
...
hype_hits="$(rg -n -i '\b(fastest|best in class|world[- ]class|ultimate|perfect|zero[- ]risk|guarantee(d)?|seamless)\b' $targets 2>/dev/null || true)"
if [ -n "$hype_hits" ]; then
...
legacy_path_hits="$(rg -n '\b(Sources|Tests|Examples|Benchmarks)/|`(sbom|checksums|security|compliance)/' $targets 2>/dev/null || true)"
